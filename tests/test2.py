...
event = address + "/getuser"
authtoken = type + " " + token
headers['Authorization'] = authtoken
res = requests.post(event,headers=headers,data=data)
if(res.status_code != 200):
  print ("Failure while getting user")
  sys.exit("user failure")
print(res.text)
print("----------------")
print("Starting Users get test")
data = {'ApiKey':'6ad','DeviceId':'test1', 'Description':'Generic event', 'message_type':'APPLICATION_EVENT_GENERIC','UserID':userid, 'Token':fbtoken}
event = address + "/getusers"
authtoken = type + " " + token
headers['Authorization'] = authtoken
res = requests.post(event,headers=headers,data=data)
if(res.status_code != 200):
  print ("Failure while getting users")
  sys.exit("users failure")
print(res.text)
print("----------------")
print("Starting Set Friend test")
data = {'ApiKey':'6ad','DeviceId':'test1', 'Description':'Generic event', 'message_type':'APPLICATION_EVENT_GENERIC','UserID':userid, 'Token':fbtoken, 'FriendID':friendid, 'Value':status }
event = address + "/setrelatio"
authtoken = type + " " + token
headers['Authorization'] = authtoken
res = requests.post(event,headers=headers,data=data)
if(res.status_code != 200):
  print ("Failure while setting friend")
  sys.exit("friend failure")
print(res.text)

print("----------------")
print("Starting Get Friend test")
data = {'ApiKey':'6ad','DeviceId':'test1', 'Description':'Generic event', 'message_type':'APPLICATION_EVENT_GENERIC','UserID':userid, 'Token':fbtoken}
event = address + "/getrelatio"
authtoken = type + " " + token
headers['Authorization'] = authtoken
res = requests.post(event,headers=headers,data=data)
if(res.status_code != 200):
  print ("Failure while getting friend")
  sys.exit("friend failure")
print(res.text)

print("----------------")
print("Starting Set Friend test")
data = {'ApiKey':'6ad','DeviceId':'test1', 'Description':'Generic event', 'message_type':'APPLICATION_EVENT_GENERIC','UserID':friendid, 'Token':fbtoken, 'FriendID':userid, 'Value':status }
event = address + "/setrelatio"
authtoken = type + " " + token
headers['Authorization'] = authtoken
res = requests.post(event,headers=headers,data=data)
